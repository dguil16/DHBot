		...
		try:
			...
			if query == 'create anon':
				if self.check_role(client, message, 'Leadership') == True:
					...
					if poll_name in all_polls["Names"]:
						yield from client.send_message(message.channel, 'There is already a poll with that name.')
					else:
						...
						f.close()
						yield from client.send_message(message.channel, 'The poll \"' + poll_name +'\" has been created.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to create polls at this time.')
			...
			if query == 'delete':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_polls[poll_name]["Author"]:
					...
					f.write(str(json.dumps(all_polls)))
					f.close()
					yield from client.send_message(message.channel, 'The poll \"' + poll_name + '\" has been deleted.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to delete this poll.')

			if query == 'vote':
				if all_polls[poll_name]["Status"] == "Closed":
					yield from client.send_message(message.channel, "That poll is currently closed.")
				
				elif message.author.name in all_polls[poll_name]["Voters"] and all_polls[poll_name]["Type"] == "Single":
					yield from client.send_message(message.channel, 'You have already voted in this poll.')
					...
					f.write(str(json.dumps(all_polls)))
					f.close()
					yield from client.send_message(message.channel, message.author.name + '\'s vote for ' + vote + ' has been recorded.')
					...
					if vote in all_polls[poll_name]["Votes"][message.author.name]:
						yield from client.send_message(message.channel, 'You have already voted for ' + vote + '.')
					else:
						all_polls[poll_name]["Votes"][message.author.name] += [vote]
						all_polls[poll_name]["Totals"][vote] += 1
						f = open('polls.txt', 'w')
			...
			if query == 'vote remove':
				...
				if all_polls[poll_name]["Anon"] == "Yes":
					yield from client.send_message(message.channel, "The votes for this poll are anonymous, so changes cannot be made.")
				elif vote not in all_polls[poll_name]["Votes"][message.author.name]:
					yield from client.send_message(message.channel, "You have not voted for " + vote +".")
					...
					f.write(str(json.dumps(all_polls)))
					f.close()
					yield from client.send_message(message.channel, "Your vote for " + vote + " has been removed.")
			...
			if query == 'admin':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_polls[poll_name]["Author"]:
					...
					f.write(str(json.dumps(all_polls)))
					f.close()
					yield from client.send_message(message.channel, message.author.name + ' has adjusted the vote for ' + vote + ' by ' + str(adjustment) + '.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to use that function.')

			if query == 'list':
				yield from client.send_message(message.channel, 'The current polls are: ' +str(all_polls["Names"]))

			if query == 'info':
				yield from client.send_message(message.channel, poll_name + ':\n\n' + all_polls[poll_name]["Description"] + '\n\nThe options for this poll are: ' + all_polls[poll_name]["Options"] +'\n\nType:' + all_polls[poll_name]["Type"] +'\n\nStatus: ' + all_polls[poll_name]["Status"] + '\n\nAuthor: ' + all_polls[poll_name]["Author"])

			if query == 'results':
				yield from client.send_message(message.channel, 'The current vote totals are as follows:\n\n' + str(all_polls[poll_name]["Totals"]))

			if query == 'open':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_polls[poll_name]["Author"]:
					if all_polls[poll_name]['Status'] == 'Open':
						yield from client.send_message(message.channel, 'That poll is already open.')
					else:
						all_polls[poll_name]["Status"] = "Open"
						f = open('polls.txt', 'w')
						f.write(str(json.dumps(all_polls)))
						f.close()
						yield from client.send_message(message.channel, message.author.name + ' has opened the poll ' + poll_name + '.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to modify this poll.')

			if query == 'close':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_polls[poll_name]["Author"]:
					if all_polls[poll_name]['Status'] == 'Closed':
						yield from client.send_message(message.channel, 'That poll is already closed.')
					else:
						all_polls[poll_name]["Status"] = "Closed"
						f = open('polls.txt', 'w')
						f.write(str(json.dumps(all_polls)))
						f.close()
						yield from client.send_message(message.channel, message.author.name + ' has closed the poll ' + poll_name + '.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to modify this poll.')

		except:
			yield from client.send_message(message.channel, 'There was an error in your request. Please try again.')
		...
		try:
			...
			if query == 'create':
				if self.check_role(client, message, 'Leadership') == True:
					...
					if survey_name in all_surveys["Names"]:
						yield from client.send_message(message.channel, 'There is already a survey with that name.')
					else:
						...
						f.close()
						yield from client.send_message(message.channel, 'The survey \"' + survey_name +'\" has been created.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to create surveys at this time.')
			...
			if query == 'delete':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_surveys[survey_name]["Author"]:
					...
					f.write(str(json.dumps(all_surveys)))
					f.close()
					yield from client.send_message(message.channel, 'The survey \"' + survey_name + '\" has been deleted.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to delete this survey.')

			if query == 'submit':
				if all_surveys[survey_name]["Status"] == "Closed":
					yield from client.send_message(message.channel, "That survey is currently closed.")
				elif message.author.name in all_surveys[survey_name]["Voters"]:
					yield from client.send_message(message.channel, 'You have already submitted an answer to this survey.')
				else:
					...
					f.write(str(json.dumps(all_surveys)))
					f.close()
					yield from client.send_message(message.channel, 'Your survey submission has been recorded.')

			if query == 'change':
				if all_surveys[survey_name]["Status"] == "Closed":
					yield from client.send_message(message.channel, "That survey is currently closed.")
				elif message.author.name not in all_surveys[survey_name]["Voters"]:
					yield from client.send_message(message.channel, 'You have not yet submitted an answer to this survey. Please use !survey-submit instead.')
				else:
					vote = survey_query.partition('; ')[2]
					all_surveys[survey_name]["Entries"][message.author.name] = vote
					f = open('surveys.txt', 'w')
			...
			if query == 'list':
				yield from client.send_message(message.channel, 'The current surveys are: ' +str(all_surveys["Names"]))

			if query == 'info':
				yield from client.send_message(message.channel, survey_name + '\n\n' + all_surveys[survey_name]["Description"] + '\n\nStatus: ' + all_surveys[survey_name]["Status"] + '\n\nAuthor: ' + all_surveys[survey_name]["Author"])

			if query == 'results':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_surveys[survey_name]["Author"]:
					for x in sorted(all_surveys[survey_name]["Voters"]):
						yield from client.send_message(message.author, x + ' submitted: \n' + all_surveys[survey_name]["Entries"][x] +'\n')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to view the survey results.')

			if query == 'open':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_surveys[survey_name]["Author"]:
					if all_surveys[survey_name]['Status'] == 'Open':
						yield from client.send_message(message.channel, 'That survey is already open.')
					else:
						all_surveys[survey_name]["Status"] = "Open"
						f = open('surveys.txt', 'w')
						f.write(str(json.dumps(all_surveys)))
						f.close()
						yield from client.send_message(message.channel, message.author.name + ' has opened the survey ' + survey_name + '.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to modify this survey.')

			if query == 'close':
				if self.check_role(client, message, 'Admin') == True or message.author.name == all_surveys[survey_name]["Author"]:
					if all_surveys[survey_name]['Status'] == 'Closed':
						yield from client.send_message(message.channel, 'That survey is already closed.')
					else:
						all_surveys[survey_name]["Status"] = "Closed"
						f = open('surveys.txt', 'w')
						f.write(str(json.dumps(all_surveys)))
						f.close()
						yield from client.send_message(message.channel, message.author.name + ' has closed the survey ' + survey_name + '.')
				else:
					yield from client.send_message(message.channel, 'You do not have permission to modify this survey.')
		except:
			yield from client.send_message(message.channel, 'There was an error in your request. Please try again.')